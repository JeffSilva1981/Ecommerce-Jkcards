...
public class OrderServiceTests {
    ...
    public void insertShouldCreateOrderWithShippingWhenValidData() {
        ...
        Mockito.when(shippingService.validateSelectedQuote(
        ...
        Mockito.when(mercadoPagoService.createPaymentPreference(
        ...
        Assertions.assertEquals(2L, result.getShipping().getServiceId());
        Assertions.assertEquals("PAC", result.getShipping().getServiceName());
        Assertions.assertEquals("Correios", result.getShipping().getCarrier());
        Assertions.assertEquals(20.0, result.getShipping().getPrice());

        Assertions.assertNotNull(result.getShippingAddress());
        Assertions.assertEquals("18000000", result.getShippingAddress().getPostalCode());
        Assertions.assertEquals("Sorocaba", result.getShippingAddress().getCity());
        Assertions.assertEquals("SP", result.getShippingAddress().getState());
        ...
        Assertions.assertEquals("18000-000", quoteRequest.getDestinationPostalCode());
        Assertions.assertEquals(1, quoteRequest.getItems().size());
        Assertions.assertEquals(1L, quoteRequest.getItems().get(0).getProductId());
        Assertions.assertEquals(3, quoteRequest.getItems().get(0).getQuantity());

        Mockito.verify(productRepository, Mockito.times(1)).findById(1L);
        Mockito.verify(repository, Mockito.times(2)).save(Mockito.any(Order.class));
        Mockito.verify(orderItemRepository).saveAll(Mockito.anyCollection());
        Mockito.verify(mercadoPagoService).createPaymentPreference(Mockito.any(Order.class));
    }
    ...
    public void insertShouldThrowDataBaseExceptionWhenStockIsInsufficient() {
        ...
        Mockito.verify(repository, Mockito.never()).save(Mockito.any(Order.class));
        Mockito.verify(orderItemRepository, Mockito.never()).saveAll(Mockito.anyCollection());
