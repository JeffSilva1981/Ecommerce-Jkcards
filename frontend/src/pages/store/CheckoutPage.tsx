...
function createCartSignature(items: Array<{ productId: number; quantity: number }>) {
  return [...items]
...
export function CheckoutPage() {
  ...
  const { selectedShipping, clearShipping } = useShippingStore();
  ...
  const orderTotal = productsTotal + (validSelectedShipping?.price ?? 0);

  const mutation = useMutation({
    mutationFn: () => {
      if (!validSelectedShipping) {
        throw new Error("A valid shipping option is required.");
      }
      ...
      return createOrder({
        ...
        shipping: {
          serviceId: validSelectedShipping.serviceId,
        },
      });
    },
    onSuccess: (order) => {
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (name === "recipientPhone") {
      nextValue = value.replace(/[^0-9()+\-\s]/g, "").slice(0, 20);
    }
  ...
  function validateAddress() {
    ...
    const phoneDigits = address.recipientPhone.replace(/\D/g, "");

    if (address.recipientName.trim().length < 3) {
      nextErrors.recipientName = "Informe o nome completo do destinatário.";
    }

    if (phoneDigits.length < 10 || phoneDigits.length > 11) {
      nextErrors.recipientPhone = "Informe um telefone válido com DDD.";
    }
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <Panel className="h-fit p-5">
          ...
          <div className="mt-5 space-y-3 text-sm text-slate-300">
            ...
            <div className="flex justify-between">
              <span>Frete</span>
              <span>{formatCurrency(validSelectedShipping.price)}</span>
            </div>

            <div className="rounded-md border border-line bg-white/5 p-3">
              <div className="flex items-start gap-3">
                <Truck size={18} className="mt-0.5 shrink-0 text-skysoft" />
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    Entrega estimada em {validSelectedShipping.deliveryDays}{" "}
